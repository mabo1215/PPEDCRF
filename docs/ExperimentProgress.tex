...
Minor. \textbf{Thirteen are closed and one --- R3 --- is closed only in the
bounded form the review itself offers}, because the published mechanism it
asks for cannot be run end to end by anyone. The cycle's substance is that two
...
closed in the bounded form the review offers, not the strong one.}
...
R3 & No published mechanism run end to end & \bounded & Closed in the review's
own bounded form, and the reason is itself reportable. GeoShield was cloned
and read line by line: \textbf{its public release stubs the vision--language
call its objective depends on}, leaving a \texttt{TODO} where the frame
description is produced. That term is \emph{subtracted} in the loss, so it is
what keeps the perturbation semantically faithful while it destroys geographic
cues, and it feeds one of the method's three named modules. Without a VLM
budget nobody reproduces GeoShield; they reproduce a different objective. So
\S III-F now says the adaptation was \emph{forced} rather than preferred,
where it used to read as a preference. A supplementary provenance table lists
every rule with its origin, and \S II carries the scope statement. Not done:
the reimplementation. Not attempted with a local VLM, because substituting one
changes the objective and the result could not honestly be labelled GeoShield. \\
\addlinespace
...
N1 & A published mechanism reproduced end to end & \blocked & R3's strong
form, and the largest remaining gap. \textbf{Blocked on the mechanism, not on
us}: GeoShield's public release stubs the vision--language call its objective
subtracts, so an end-to-end run needs a VLM API budget and would still be
reproducing the authors' method only up to that substitution. The manuscript
bounds its negative claim to the enumerated rules and prints their provenance,
which is the review's stated alternative. Reopen if a public release with that
component lands. \\
\addlinespace

N2 & A stronger geolocator than GeoCLIP & \pending & A PIGEON-class model
would test whether the direction result survives a better image-to-GPS
attacker. GeoCLIP was chosen for public weights and modest memory, and it
passes the capability gate ($54.5\%$ within 25\,km, median $18.4$\,km), so the
arm measures a movable benchmark rather than a floor. The finding is stated
for the model tested. \\
\addlinespace

N3 & Adaptive attacker at a larger budget & \pending & A fully unfrozen
encoder, data beyond this benchmark, or re-indexing at every step. The
manuscript states the budget it could afford and confines its conclusion to
it. \\
\addlinespace
