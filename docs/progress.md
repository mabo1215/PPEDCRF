...
509. 【进行中】**MixVPR 上补齐两个新实验**（R1 的 clip-pooling 和 R6 的 purification
     此前都只在 ResNet18 上做）。缓存已建完（2,892 个释放），
     clip 跑到约四分之一，purification 正在训第一个净化器。
...
- **【无需决策，R6 已完成】** 净化攻击者跑完并已回填论文，**PRO 6000 已关机**。
  唯一还可以再做的是把同一套净化实验搬到 MixVPR（强攻击者）上——
  那边 direction 的效应本来只有 −0.048，净化很可能整个抹平，
  值得知道但需要再开一次卡（约 40 分钟）。要跑的话说一声。
