...
- **【新增待决策 2026-09-13 晚】main.tex 现在是 14 页，超出 13 页限制一行。**
  R3 的结果写进正文后，正文从 13 页涨到 14 页，**第 14 页上只有一条参考文献的续行**。
  我做了四轮压缩（都只删冗词、不删任何 hedge／caveat／否定结论）都没能把它拉回来，原因是
  **第 12 页被浮动体占满、正文只有 5 行**，所以第 12 页之前的删减会被浮动体重新排布吸收掉，
  并不会把参考文献往上顶。
  两个选项：(a) 接受 14 页并支付 IEEE 超页费；(b) 从正文里删掉约一栏的实质内容。
  我的建议是 **(a)**：能删的冗词已经删完，再删就要动 hedge、caveat 或否定结论，
  而这些正是这篇论文的立论所在——为了省一页把它们削弱，代价远大于超页费。
  A:
