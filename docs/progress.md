...
- E1（真实 place/GPS VPR）【进行中】：MSLS 数据已下载并按需抽取 Manila+Toronto 两城图像（约 31600 张，已核对完整）；`build_msls_manifest.py` 正在本地运行中（CPU/IO 密集，无需 GPU），完成后需再跑 gate 检查和 `run_geotagged_vpr_benchmark.py`。
- E5（unary map 独立验证）【已阻挡，需要你的操作】：KITTI-360 数据+manifest 已就绪，诊断已在真实数据上跑过并确认 gate 不通过（checkpoint 空间标准差约 $2.6\times10^{-4}$，与代理数据结论一致）；下一步需提供 mask-backed checkpoint 或重新训练。

# 遗留问题

- E1 公开数据集注册【已解决数据获取，manifest 构建中】：MSLS 的 11 个签名链接已由你提供并全部下载完成（`G:\work\datasets\msls\raw`，10 个 zip 齐全），Manila+Toronto 两城图像已按需抽取解压（约 31600 张，已核对完整）。`build_msls_manifest.py` 正在本地运行（纯 CPU/IO，不需要 vGPU），完成后跑 gate 检查和 `run_geotagged_vpr_benchmark.py`（同样在本地跑，不需要远程 GPU），无需你进一步操作即可继续。
- KITTI-360（E5 数据获取部分）【已解决】：你已确认拥有 cvlibs.net 授权，我们改用公开 S3 镜像直接下载了所需的 2 个 sequence 的 perspective images（`image_00`）加全量 semantics/poses，未再使用 `download.php` 的 token 脚本方式，`G:\work\datasets\kitti360\download_scripts\` 无需再放文件。E5 诊断本身仍受 mask-backed checkpoint 阻塞，见下一条。
