...
- **【下一轮第一件事，R1】在 KITTI-360 的同一个 manifest 上跑一次 clean pass**
  （不加扰动），把 clean Top-1 印在扰动值旁边；白盒臂同样便宜的话一起跑。
  **本机几分钟，不需要新数据、不需要远程 GPU。**
  跑完看数：clean 明显高于扰动值，则 null 有锚、"replicates" 站得住；
  若差不多，就要老实说"这个数据集没有足够的检索量来检测 allocation 效应"
  并相应收窄复现主张。
- **【R2】** 把 605/605 与 1,211 两个数一起印出来。**不需要计算。**
- **【R3】** 重写补充材料开头那一段，说清楚哪张表在哪个文档。**不需要计算。**
- **【R4/R5】** 补一句 clustering 代价与 queries-per-place 的关系；
  把八条/十三条放置的口径对上。
