...
- **暂无阻塞项。** 第八轮 R1--R7 全部闭合，三条 presentation note 也都落地。
- **【不阻塞，下一轮候选】R5**：放置集合里仍然没有第三方训练的 selective
  sanitizer 跑过协议。结论已限定范围，不算过头主张。
- **【不阻塞】KITTI-360 只有 16 个 place**，要让 direction 在第二个数据集上
  真正分离，需要 place 更多的语料（更多 drive 的影像，或另一个数据集）。
  现在的措辞已经如实说明了这一点。
