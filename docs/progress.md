...
- **【要你确认，R5】用 KITTI-360 代替评审点名的 Tokyo 24/7 / RobotCar，可以吗？**
  理由：它是真正换了数据集、城市、相机和拍摄方式（车载、Karlsruhe），
  而且它的 poses 支持和 MSLS 同一条 25 m 判据，现有的 manifest gate 和
  "cluster vs geographic" 审计能直接搬过去；项目里也已经有它的 loader。
  要建的是：从 poses 生成 place-labelled manifest（按 drive/时段切 query/gallery，
  25 m 内为正样本，gallery 规模对齐 MSLS 的 2000），然后跑两件事——
  八放置能量匹配 + direction vs isotropic，一个骨干、一个预算、带 place-clustered 区间。
- **【要你确认，R5 的机器】建议就在本机 3070 上跑，不用开远程。**
  数据在本机 G: 盘，工作量也不大（放置臂是 8×3×400 次前向；
  direction 臂约 1200 次 20 步 sign-gradient）。
  搬一份 KITTI-360 子集去 AutoDL 是另一条路，但纯属多此一举。
- **【现在就可以做】2c 和 vGPU 3090 关机**，两台对这篇论文都用不上了。
