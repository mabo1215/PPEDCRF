...
## 本轮追加二（2026-09-09 夜，vGPU 3090 无卡模式：tifs_d6 找回，审计器 95 → 130）

383. 【已完成】**vGPU 3090 上有 `tifs_d6`，这是三棵里最大也最要紧的一棵**。
     - 上一轮（第 343 条）说"vGPU 全盘 find 搜过，十二棵树一棵都不在它上面"——
       **那个结论对当时那份清单成立，但对现在这三棵不成立**，因为要找的东西变了。
       主机状态变了就该重搜一遍，不能拿旧结论当结论。
     - checkout 只有一个：`/root/autodl-tmp/ppedcrf_tomm_20260830/PPEDCRF`。
       全盘 `find /` 之后拿到 **`tifs_d6`（20 MB / 12 个 CSV）** 和
       **`tifs_d7`（9.5 MB / 4 个文件，释放帧的下游检测与分割效用）**，
       两棵在别处都不存在。
     - **审计器 95 → 130 verified，仍然 0 mismatched。** 一次补上 **35 条**：
       Table IV 两个攻击者的迁移列和白盒行、Top-1/5/10 的 ranks 段，
       **全部对着原始 per-query 行验过**，这是本会话摸得到的机器上第一次。
     - `tifs_d6` 带了个 `superseded/` 子目录（早期 EOT 跑，被旁边三个按种子分的
       文件取代）。**照原样保留**：它记录了什么被替换掉了，而且所有消费方都只
       glob 树的顶层，读不到它。

384. 【还缺两棵】**`tifs_a8` 与 `tifs_a8_hi`**（操作点 15.68 和高预算 241.5 的
     释放边界），撑着最后 **16 条** claim。
     - **2c 和 vGPU 都做过全盘 find，两台都没有。** 它们是在 PRO 6000 上跑的
       （第 350 条），所以只剩 **PRO 6000 或工作机** 两个地方可找。
     - **如果找不到，重跑是可行的**：vGPU 上 `data/msls/manifest_all8.jsonl`
       在，torchvision 的 resnet18/resnet50/vgg16/deeplabv3/fasterrcnn 权重
       也都在缓存里（只差 cosplace，可下载）。**但 vGPU 现在是无卡模式**
       （`/dev/nvidia*` 不存在），要开卡才能跑 A8。

385. 【教训】**"某台机器上没有"这个结论会过期**。第 343 条那句话我这轮差点直接
     信了。主机重开、清单变化之后都要重搜；而且要搜**全盘**、要枚举**所有
     checkout**——2c 那次就是只搜了主 checkout 才漏掉 `tifs_a7b`。

## 本轮追加三（2026-09-09 夜，PRO 6000：最后两棵找回，审计器 153/153，并查出正文一处真错）

386. 【已完成】**PRO 6000 上有 `tifs_a8` 与 `tifs_a8_hi`，外加 `tifs_a2`**。
     `tifs_a2` 撑着 R2/R3 的那批数（列范数集中度、Spearman、翻转率），
     **本会话摸过的机器里一台都没有**，这次才拿到。
     **审计器：153 条 claim，153 verified，0 mismatched，0 unverifiable。**
     十三族导出第一次在同一台机器上齐全。

387. 【查出正文真错】**高预算那一档的释放边界，正文引的是跑偏那版的数**。
     - PRO 6000 上同时有 `tifs_a8_vitstart8` / `tifs_a8_hi_vitstart8`
       （代理集 `vit_b_16`、`random_start` 8.0，即第 347 条说的跑偏配置）。
     - 正文写 `g=2.312`、`max|δ|=36.99`——**这正是跑偏那版的数**；
       按已发表配置（cosplace / random_start 1.0）应为 **3.078 / 49.25**。
     - **同一句话里的操作点那半（0.773 / 12.36）用的却是正确那版**，
       也就是一句话里混了两个版本的实验——**正是 R1 点名的那类缺陷**。
     - 第 352 条重跑后只更新了操作点，高预算那档的数没跟着换。
       **树在 PRO 6000 上读不到，所以扛过了五轮评审都没被发现。**
     - 已改正，正文与审计器同步。**结论不变，而且更强**：49.25 比 36.99 离
       ℓ∞ 投影 16 更远，"clearly outside"照样成立。
     - **两棵跑偏的树照样提交保留**，这样这处更正可以拿 `run_config.json`
       对着查，而不是让读者信我一句话。

388. 【查出第二处，属于工具缺陷】**`tifs_a2` 的分析脚本把重启产生的重复行算了两遍**。
     producer 是可恢复的（第 355 条那条铁律），重启会把当时在跑的 query 再写一行——
     628 行里有 **28 行**是这么来的，而 `analyze_jacobian_columns.py`
     直接对原始行求平均，等于给这些 query 加了双倍权重。
     - 已改成**按 `(query, map)` 保留最后一行**，并在报告里输出被重启取代的行数，
       **让汇总结果与"重启过几次"无关**。
     - 正文随之微调：**153 → 150 query**，集中度 **0.477→0.478**、**0.653→0.654**，
       Spearman **0.656→0.659**，上限 **0.982→0.984**，
       score-gradient 翻转率 **9.2%→9.0%**。uniform 翻转率两种读法都是 **5.0%**，
       所有结论不变。
     - **A2 的七个数已注册进审计器**——P3 的最后一项也关掉了。

389. 【教训】**"数字对得上"要对得上**正确那次运行**，不只是对得上某次运行。**
     第 352 条当时确认了操作点回到发表配置，就默认整节都更新了，
     其实高预算那档没跟着改。**重跑之后要把该次实验的每一个数都重新核一遍，
     不能只核你当时在看的那一个。**

## 本轮完成（2026-09-09 夜，压页收官：13 页达标）

390. 【已完成】**main.pdf 15 → 13 页，达到 TIFS 上限。** 补充材料 6 页（无页数限制）。
     摘要 **243 词**（上限 250）。两个文档编译均 0 undefined reference。
     **审计器 153/153 verified、0 mismatched、0 unverifiable。**

391. 【教训：我的页数估算错在哪】**"搬走 §Which Part 能省 0.79 页"是错的**。
     0.79 页是**整节的体量**，但我搬走之后在正文留了 2,235 字符的摘要，
     实际只省了一半。
     - **正确做法是直接量最后一页**：用 pypdf 读 PDF，页 14 只有 **85 行**、
       全是参考文献，而满页约 **137 行**。由此得到 **约 61 字符/排版行**，
       剩余缺口从"估计"变成"可数"。以后压页一律用这个口径，不要再用 6,107 字符/页
       去推，那个数在有浮动体的时候不准。
392. 【已完成】**最终搬走的三样，按贡献排序**：
     ① §Which Part of the Perturbation Does the Work（含 Table VI）
     ② 预处理那张图 `fig_preprocessing_eot`（省 27 行）
     ③ mask-guided 表（省 16 行）
     **解释这三样的正文散文和它引的每一个数都留在正文**——
     搬走的只是"展示物"，不是论证。R6 的三因子结论、R4 的 mask-guided 对照结论，
     正文都还在。
393. 【已完成】**顺手修掉搬运带来的三个质量问题**：
     - 搬过去的图和表**落成了没有归属章节、也没有任何引用的孤立浮动体**——
       正是 R13 反对的那种。图已归入补充材料的 §Preprocessing 一节，
       表新开一节 §Mask-Guided Perturbation, Full Table 并在正文引用。
     - 补充材料原来写着"Figure~3 of the manuscript"，**图搬过来之后这句话就错了**，
       已改成引用本地图号。
     - `fig:protocol` **本会话之前就一直没有被引用**（不是我改坏的），一并补上。
       现在两个文档**所有浮动体都有引用**。
394. 【已完成】**审计器的 source 跟着数字走**：factorial、mask-guided、
     reproduction-gate、clamp 四组 claim 的 `source` 全部改成 `supplementary.tex`，
...
## 第六轮实验收尾（2026-09-10 凌晨，PRO 6000 十任务跑完 + 关机）

411. 【已完成】**10 个任务全部 EXIT=0，结果已拉回并验证，PRO 6000 已关机**
     （端口不再响应，确认停止计费）。拉回 1.7 MB，本地行数与远端逐一核对一致。
412. 【已完成】**R5 收口**：frontier 与释放边界从**单种子**变成
     **效用 3 种子 / 隐私 2 种子 400 query**，Table V 每格都有 bootstrap 区间。
     **结论有实质变化，而且对论文更有利也更准确**：
     0.05 容差下**只有 1 格整段区间达标**（isotropic@5.0，但它什么也没做，
     Top-1 0.210 对 clean 0.197），**另有 5 格跨线**，其中就有操作点的 direction
     （Top-1 **0.041**，drop 0.098 [0.047,0.144]）。
     所以旧那句"容差内没有任何机制有效"**不再成立**，改成"操作点是否负担得起，
     在这个容差下是**未决**而不是被否定"。
413. 【已完成】**R3 收口**：`vit_b_16` 作为**代理集里不存在的 trunk** 的留出攻击者。
     direction **−0.0233 [−0.0428,−0.0056]**、加固 **−0.0358 [−0.0662,−0.0066]**，
     两个区间都不含零 —— **跨架构确实成立**；但比 ResNet18 的 −0.165 **小一个数量级**，
     而 ResNet18 正好和 CosPlace 代理同架构。白盒仍能把它打到 0.0000。
     **正文据此明说：之前报告的迁移强度大部分依赖"攻击者与某个代理同 trunk"。**
414. 【查出第三处版本混用】**已发表的释放边界四档不是同一个配置**：
     `run_config.json` 显示 MSE **5.0 和 60 用的是 random_start 8.0**，
     而操作点和 241.5 用的是 1.0。MSE 60 因此从 g=1.146 变成 **1.516**
     （max|δ| 18.34 → 24.26）。正文改用**四档同配置**的新运行；
     15.68 与 241.5 两档新旧配置相同、数值也吻合（0.7727 vs 0.7727；3.078 vs 3.081），
     这就是"其余条件没变"的对照。
415. 【查出我自己工具的漏洞】**审计器的"printed string 是否还在源文件里"这一检查，
     之前只加载了 MAIN 和 TAB_TRANSFER 两个文件**，
     其余 source（补充材料、生成的表）`sources.get()` 返回 None，
     而代码把"取不到文本"当作"找到了"，于是**静默跳过**。
     也就是说 factorial、mask-guided、reproduction-gate、clamp、frontier
     这几族的 locator 检查**从来没真正跑过**。改成加载 claim 声明的**全部** source
     之后，立刻抓出 **12 条过期的 frontier claim**。
     **更正**：我之前说"审计器确认没有 claim 从它所在的源文件里消失"，
     对这几族而言并不成立——那是工具没查，不是查过通过。
416. 【状态】**168 条 claim / 168 verified / 0 mismatched / 0 unverifiable**，
     正文 **13 页**、补充材料 6 页、摘要 243 词、0 undefined reference、
     所有浮动体都有引用、两个文档 0 个代码标识符。

## R9 联合测量（2026-09-10 上午，PRO 6000 第二次开卡，12 任务，已关机）

417. 【已完成】**R9 的结构性批评用测量回答了，不是靠改措辞**。
     新脚本 `evaluate_joint_utility_msls.py`：在**检索攻击评的那 400 张 MSLS query 帧上**
     跑冻结分割器。4 档预算 × 3 种子 × 400 帧 × 4 条件 = **19,200 行，12 个任务全部 EXIT=0**，
     约 35 分钟跑完，结果已拉回并核对，**PRO 6000 已关机**（端口不再响应）。
     - **MSLS 没有分割标注**，所以参照物是分割器**自己在干净帧上的预测**，
       量的是 **agreement 不是 accuracy**——满分只说明释放帧对分割器不可见，
       不说明分割器判对了。脚本 docstring、run summary 和正文都写明了这一点。
     - **操作点结果**：isotropic 改变 **1.7%** 像素 [1.4,2.1]，
       direction **3.3%** [2.8,3.8]，加固版 **5.4%** [4.7,6.2]，**三个区间互不重叠**。
       **这正是 VOC 上那个次序**，现在落在和检索同一批帧上。
418. 【教训：两次统计口径错误，都是被 clean 那一行抓出来的】
     导出数据一个字没变，错的是我算的统计量：
     - **按类平均 IoU 会被稀有类绑架**：释放帧凭空多出来的一个几像素的类，
       和占半张图的类**权重一样**。这么算操作点的"agreement"是 0.19，
       看着很吓人，其实没有意义。
     - **第一版按像素算，我除以固定的 192×320**，但分割器自己的 transform 会缩放，
       于是 clean 那行算出 **7.33**（必须是 1）。
     - **正确做法**：每个像素在两张图里各属一个类，配对的贡献 I+1、U+1，
       不配对的贡献 0 和 2，所以像素数 = **(I+U)/2**，
       每像素一致率 = **2I/(I+U)**——不需要知道分割器内部分辨率，
       clean 行必然等于 1。现在 clean 行读数**正好 1.0000**，
       并作为"流水线确定性"的闸门保留在表里。
     - **`clean` 这一行的价值就在这里**：它是唯一一个答案已知的格子，
       两次口径错误都是它先报警的。
419. 【状态】**172 条 claim / 172 verified / 0 mismatched / 0 unverifiable**；
...
## E1 导出树补审计（2026-09-10，你说"给 E1 导出树写一族 claim"）

438. 【已完成】**E1 那 36 格现在全部进了审计器：331 → 333 条 claim，全部 verified。**
     两张 E1 表的每个 raw / PPEDCRF / Δ（36×3）、两城表的 18 个区间上下端（36 条）、
     "18 格里 5 格区间不含零"、以及两张表 caption 里的负号计数，都从导出重算。
439. 【已完成】**先把数据变成"能跟着仓库走的"**：E1 的运行结果原本只在
     `src/outputs/`（不提交），fresh clone 根本无法复算——这正是它一直没被审计的原因。
     新脚本 `export_e1_claim_rows.py` 只抽验证需要的五列
     （query_id / place_id / variant / seed / correct_rank），
     **16 MB → 2.5 MB**，提交进 `src/exports/e1_msls_rows/`。
     Top-1/5/10 都能从 rank 还原，没丢任何表里报的东西。
     目录名故意不叫 `e1_msls`：`src/outputs/e1_msls` 已经被另一棵无关的树占了，
     而审计器两个根都搜——同名两棵树会让"导出缺失"读成"导出存在"。
440. 【查出一处真问题，也是这次最值得记的】**两城表的区间列，有两个端点是复算不出来的。**
     36 个点估计全部**逐位复现**（0.1700 / 0.1517 / −0.0183 …），
     但 `old→new / Patch-NetVLAD` 的下端（印 −0.139）和
     `new→old / VGG16` 的上端（印 +0.093），
     **在 24 个随机种子、两种 seed 平均口径、2000 与 10000 次重抽下都够不到**，
     偏离约 0.005，而同一张表里 155 簇的格子种子间只抖 0.0009。
     也就是说**这不是蒙特卡洛噪声，是系统性差异**。生成它的脚本当年没提交，
     所以差在哪已经查不回来了。
441. 【已完成】**处理办法不是把容差放宽到能过，而是把这张表改成生成的。**
     新脚本 `make_e1_tables.py` 从导出重算两张表并写进 `paper/generated/`，
     补充材料改成 `\input`。区间用**固定种子 + 10000 次重抽**
     （54 簇那几格 2000 次时端点种子间还抖 0.002，和表要读的差值同量级）。
     **两句正文结论都没变**：仍是"14 of 18 为负"、"5 格区间不含零"，
     而且**被标星的还是原来那 5 格**。
442. 【生成器自己又抓出一个小错】wide-8 表的 caption 我让它自动数负号，
     第一版印"16 of 18"，但读者照着表数只有 15。
     原因：`old→new / CosPlace` 的 Δ 是 **−2.2e-18**，
     印出来是 `±0.0000`，用原始浮点数判 `<0` 就把它算成负的了。
     改成**按印出来的位数四舍五入后再判**（区间是否含零同理），caption 与表一致。
443. 【状态】主文 13 页 / 补充 6 页 / 扩展报告 12 页，四份文档 0 overfull、
     0 undefined reference/citation；**审计器 333/333**。
     两个文档里**已经没有任何一个印出来的数字是审计器不覆盖的**。

## 第七轮独立评审（2026-09-10，你触发：按 TIFS 要求完整评审并覆盖 RevisionSuggestions.tex）

444. 【已完成】**第七轮独立评审已整段覆盖写入 `docs/RevisionSuggestions.tex`（11 页，编译 0 错误 0 overfull）。**
     按协议：不以上一轮评审、progress.md、任何"已完成"判断为输入，
     直接对 `main.tex`（sha `1df4fd3d…`，13 页）、`supplementary.tex`
     （sha `2074fabb…`，6 页）和两份 PDF 审，需要时直接读 `src/` 的代码核实。
     **结论：Minor revision。** 上一轮挡住的四件事（可达性、两文档不同步、
     架构表述不实、单种子）确认真的解决了，而且是能在源码里看见的解决。
445. 【本轮扫出的四个真问题，都是"顺着一条引用走过去发现走不通"】
     这四条不改任何结论，但审稿人恰恰是这样查论文的：
     - **R1 指向落空**：正文写"补充材料给出学习率扫描、per-condition 表、
       以及 MixVPR 的同样分析"，**补充材料里只有 per-condition 表**；
       另一处"MixVPR 八种放置的 per-placement 值在补充材料"——**也没有**
       （S4 是三个重分布控制，S5 是代理语料上按六骨干池化的）。
       讽刺的是这正是上一轮 R2 那类缺陷，**被 R2 的修复本身重新引入**。
     - **R2 表号错了**：正文指"Fig. S1 and Table~S7"看预算扫描，
       但两张 E1 表合并后编号整体前移，**预算扫描现在是 S6，S7 是 VOC/COCO 效用表**。
       而且 `main.tex` 自己的注释就禁止跨文档 `\ref`——所以这条**任何自动检查都看不见**：
       编译 0 undefined，审计器只查数值不查指向。
     - **R3 星号打在没被检验的量上**：Table I 的 `learned support` 是
       `+0.039^\ast`，而 caption 说星号=query 为单位下显著；
       但 `+0.039` 是**六骨干三种子两基准池化后的均值**，
       真正被检验的是正文里 VGG16 那个 `+0.194`（12 query、p=0.05、98 次比较里的探索项）。
       补充材料 S5 同一个 `+0.039` **没有星**——两张表对同一个数字说法不一致。
     - **R4 那句校准没有出处**：两个文档都写"real 的效果比 proxy 小得多
       （Top-1 drop 0.018 versus 0.111）"，**这两个数在任何一张表里都找不到**。
       从导出重算：8 城表 primary 六骨干均值是 **0.0186**（印出来该是 0.019）、
       ResNet18 格是 **0.0142**，而 **0.018 恰好等于两城表的 ResNet18 格**——
       补充材料把一个**两城的数挂在了 8 城表下面**。0.111 更是任何表都推不出来。
446. 【另外两条是科学范围，不是错误】
     - **R5 只有一个真实检索数据集**：所有 real-place 结论都在 MSLS 一个数据集、
       且论文自己承认 query "几乎全是正视角、白天、季节天气字段为空"。
       否定性结论只有单数据集证据，是审稿人最自然的攻击面。
       **`ref.bib` 里躺着 Tokyo 24/7 和 Oxford RobotCar，两条都从未被引用过**——
       正好是补这一条最省事的两个候选。
     - **R6 放置集合里没有任何第三方训练出来的选择性 sanitizer**：
       测的是我们自己指定的先验、梯度图、三个被拿来当放置先验的分割模型，
       加上我们自己的两个 checkpoint（其中一个还是退化的）。
       "no placement beats uniform" 的实际范围比措辞给人的印象窄。
447. 【自我更正】上一轮我跟你说"两个文档里已经没有任何一个印出来的数字是审计器不覆盖的"，
     **这句话是错的**。审计器是**声明式登记表**，不是全量扫描：
     main.tex 里 3--4 位小数的字面量有 182 个不同值，**其中 74 个在审计器里找不到对应**
     （多数是正文里引的区间端点、KITTI-360 那三个归因数、以及 R4 那两个数）。
     333 条 claim 覆盖的是**两份文档里所有表格单元格**，这句才是准确的。
448. 【状态】评审文件本身 11 页、0 overfull、0 undefined；论文未改动，
     仍是 13 页 / 6 页、审计器 333/333。

# 遗留问题（第七轮评审之后）

- **【要你决定，R5/E4】第二个真实数据集要不要做。** 这是新评审里**唯一需要 GPU
  的一条**，也是唯一需要你拍板的：Tokyo 24/7 或 Oxford RobotCar 本机有没有？
  要跑的话只需要一个骨干、一个预算，跑八放置 + direction 对照两件事就够，
  几个 GPU 小时。**不跑也有出路**：把"regime"级别的措辞全部收紧成
  "MSLS、白天正视角"，代价是论文的一般性主张变弱。
- 其余九条（R1--R4、R6--R10）**都不需要你决策**，是文档工作加两处脚本，
  我可以直接推进。
