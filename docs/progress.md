...
- E1 条件多样性扩展【可选后续】：当前 MSLS 结果只覆盖 day/Forward-view，未覆盖 Design.md 原计划的 illumination/season/weather 变化；下一步可用 `--subtask o2n`（Manila/Toronto 均有实际标注覆盖）补做一次条件对比，属于加分项而非阻塞项，论文已如实注明该局限，不需要你现在决策。
