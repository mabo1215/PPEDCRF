...
**同日第三轮：submit/ 只留上传件（2026年9月15日）**
- `00_MANIFEST.md` 移到 `docs/`，`submit/04_cover_letter.txt` 删除，内容合并进 `docs/cover_letter.txt`（那份原来落后一版）。
  投稿信本来就是在系统里填写而不是从 `submit/` 上传，所以它与清单一起放在 `docs/`。
- 顺手改掉投稿信里一处不实表述：原文说 extended evidence report 与数据一起存放在 DataPort，实际存档里没有这份文档（见下方遗留问题）。
- 问卷 Q2–Q5 按你确认的记为 **No**（清单里的长篇分析与可直接粘贴的 500 字符答案现在一致）。
...
另有 `arXiv:2603.01593`（2026-03-02，五位作者）是提出 PPEDCRF 机制的更早一篇。已据此调整 `docs/00_MANIFEST.md`：
- Q1 答案加上 “posted as arXiv:2604.17163”（487/500 字符）；
- Q4 仍答 **No**——问的是「与投稿稿**相同**、仅有细微差别」的预印本，而那版是审计前的版本（标题不同、2,992 vs 11,461 词、结论相反）；
- Q5 由 No 改为 **Yes 并列出两条**（434/500 字符）；
- 投稿信也补了一句披露。
- 仍需你定的一点：`arXiv:2603.01593` 如果**此刻正在别处投稿或已被接收**，Q3 就要改成 Yes 并列出它；只是挂着预印本的话维持 No。

## 数据资产与 DOI 已确认（2026年9月15日）

数据资产 `ppedcrf-evidence` 已挂在 capsule 上，所以一键运行会真的重算断言而不只是跑合成自检；
DOI 就是 IEEE DataPort 那个 `10.21227/jnr0-jm15`，没有单独的 Code Ocean DOI——稿件脚注本来就同时给了这两个指针，不用改。

## `main.aux` 决定不补（2026年9月15日）

你说得对，它确实是中间产物那一类，所以按「源码包只放源文件」的原则不补回。
代价记录在案：补充材料单独编译时有 **17 处指向正文的引用变成 `??`（6 个不同的 section 标签），且编译仍 exit 0**。
上传的 `02_supplementary.pdf` 是带着正确编号编出来的，审稿人读的是它；真正会碰到这问题的是录用后 IEEE 排版环节，
届时若要源码自洽，干净的做法是去掉 xr、把那 6 个 section 编号写进补充材料正文。
