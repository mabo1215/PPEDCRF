...
179. 【已完成】**第三个独立分割放置：SegFormer-B0 / ADE20K**。此前只有 DeepLabV3 一个已发表模型，容易被质疑"结论依赖某一个网络"。ADE20K 直接命名了建成环境（building/sky/road/tree/sidewalk/wall 等 13 个场景结构类），把这些类概率求和作为权重图，比 VOC 的单一泛化 `background` 类更贴近"扰动背景场景"这一已发表策略的本意。核实过它确实是**独立规则而非换皮**：两族图在真实街景上相关性仅 0.55–0.75，CV 也更高（0.10–0.22 vs 0.08–0.17）。结果：Δ=+0.0000，p=1.00，CI [−0.009,+0.009]，n=1200。
