...
## 第十二轮独立评审与修订（2026-09-11，你触发：再次评审看达到什么水准）

543. 【评审结论】通读正文与补充材料后的判断：**从 minor revision 区间继续上移，
     卡点已不在严谨性上**。方法论强度高于 TIFS 同类防御论文平均线——交付口径失真对齐、
     预注册等价边界、place-clustered 与 query-level 并报、主族/探索性事先声明加 Holm、
     自适应攻击做到净化器和文件级发布。正文两处自我推翻（自己的图是常数、
     "分配轴无效"被 solved map 推翻）是加分项，因为它先把稻草人自己指出来了。

544. 【已全部修改】**摘要补上最不利的事实。** 原摘要推荐 direction 时，
     只说它"在同等失真下比各向同性噪声多花效用"。但 Table I 更狠：
     12 个 cell 里只有 1 个在宽松容差下整条区间可用（MSE 5 的 isotropic），
     而它把 Top-1 从 0.210 移到 0.210，**等于什么都没做**。
     即**不存在同时可用且有效的操作点**。正文容差段本来就是诚实的，
     但摘要是审稿人先读的那一半，不能比正文温和。现已写入。

545. 【已全部修改】**检索攻击者的边界从结论前置到 scope 段。**
     image-to-location 模型（读图直接吐坐标）被威胁模型接纳但证据覆盖不到——
     这句原先埋在结论倒数第二句，现在移到 §I scope 段，结论里的长版本压成一个从句。

546. 【已全部修改】**补两条文献，堵住时效性缺口。** 原来 2024 有 4 篇、2025 零篇、
     2026 一篇，覆盖基本停在 2024。新增 Luo et al.（ICLR 2026，多模态推理模型的
     位置泄露）和 Malone et al.（2025，VPR 自身的对抗攻击与检测）。
     **两条都逐一核实过标题、作者、arXiv 编号和收录情况，不是凭搜索摘要写的。**

547. 【方法记录】加完文献后第 13 页的参考文献溢出一条到第 14 页（4 行）。
     重建了"段末行检测器"定位最便宜的删改点：按列收集行的 bbox，
     判定下一行是否缩进来识别段末行，再按段末行已用宽度排序——用得越少越便宜。
     全文 123 个段末行里最便宜的四个只需删 10--26pt 宽的内容就能各掉一整行。
     **踩过的坑记下来：`pdftotext` 默认只打印前 30 条，之前统计一直只看到 2 个候选，
     以为是检测器坏了，其实是输出被截断。**

548. 【当前状态】正文 13 页、补充材料 6 页、摘要 249 词、**827 条 claim 全绿**、
     0 overfull、0 undefined、0 LaTeX 错误、65 条参考文献。已提交并推送
     （paper 子模块 737ea41）。**第 13 页余量只剩约 5pt**——比上一轮的 67pt 更紧，
     再加任何一句都会顶到 14 页，必须同步砍字。
...
- **【无需决策，已完成】R1 的补充实验已在 vGPU 3090 跑完并写进论文**（第 561--564 条）。
  ...
  证据面更宽而主张更保守。**卡可以关了。**
- **【无需决策，可选】** R3 的收敛曲线仍未做：现在只有 20 步和 40 步两个点，
  而五条臂在 40 步都还在长。再跑两三个步数能把"20 步预算下买不到"变成一条曲线。
  锦上添花，不是必需项。
