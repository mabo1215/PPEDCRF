...
501. 【进行中】**R1：clip-pooling 攻击者（拿到整段视频的攻击者）正在 PRO 6000 上跑。**
...
505. 【页数】正文 **13 页**（前后重排了八次）/ 补充 6 页 / 摘要 **243** 词 /
     四份文档 0 overfull、0 undefined；auditor **432/432**；跨文档引用 0 broken。
     "把'哪些 query 会被各向同性预算打动'那段移进扩展报告"是这次腾页的最后一步。


# 遗留问题

- **【进行中，R1】** clip-pooling 实验在 PRO 6000 上跑，进度可在
  `src/outputs/clip_pooling/` 看行数。**跑完会拉回结果、回填论文，然后按你的要求关机。**
  唯一还需要你决定的是：**如果 pooling 把 direction 的效果收回去很多**，
  正文的推荐要不要从"direction 是可用的那条轴"改成"单帧场景下可用、
  视频场景下需要跨帧一致的方向"——这会动摘要和结论的措辞。
- **【可选，R6】** purification 攻击者（训一个去噪器）本轮**只写了限定，没有跑**。
  要跑的话是个小实验（§III-J 的 split 和 release 都现成），但需要再占一次卡。
