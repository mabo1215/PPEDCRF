...
def transfer_table(summaries, held_out, out):
    lines = [
        ...
        r"stronger. $\Delta$ is paired against the isotropic control, seeds",
        r"averaged within a query, with a query-level bootstrap interval and a",
        r"Wilcoxon signed-rank test; \S\ref{sec:direction} gives the",
        r"place-clustered intervals. All four attackers are held out from the",
        r"optimiser, the lower two from the surrogate trunks as well, and the",
        r"trunk of the last appears in no surrogate at all.}",
