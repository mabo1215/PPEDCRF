...
def main() -> int:
    ...
    for tag, pats in families.items():
        ...
        for n, src in cells:
            ...
            curves.setdefault(tag, []).append((n, d.mean(), lo, hi, dec, obj, src))
    ...
    if args.latex:
        ...
        # A budget measured by two runs is reported from the original one --
        # the value the manuscript already prints and the registry already
        # carries -- and the re-measurement becomes the reproduction spread
        # quoted in the caption rather than a second number in the column.
        spread = 0.0
        ...
        # "at most" has to be true after rounding, so round up rather than to
        # nearest: the largest spread is 0.005833 and a caption claiming 0.0058
        # is claiming a bound the data breaks in the fourth decimal.
        import math
        spread = math.ceil(spread * 10000) / 10000
        lines = [
            ...
            r"query clusters and three seeds per cell; bold is a "
            r"place-clustered interval excluding zero. Twenty is measured by "
            rf"both runs and they differ by at most ${spread:.4f}$, which is "
            r"what puts the five points on one trajectory; the column reports "
            r"the first. Every arm but MixVPR's is solved against the same "
            r"three surrogates and is therefore the same map, MixVPR being the "
            r"evaluation target in its own arm.}",
        ...
        for tag, rows_ in curves.items():
            by = {}
            for b, d, lo, hi, dec, obj, src in rows_:
                # prefer the original run over the re-measurement
                if b not in by or (src.startswith("r14_") is False):
                    if b in by and not src.startswith("r14_"):
                        by[b] = (d, lo, hi)
                    elif b not in by:
                        by[b] = (d, lo, hi)
            cells = []
