...
SNAP=/mnt/d/source/PPEDCRF/src/scripts/_remote_snap_eval.sh
...
while true; do
  ...
  if [ -z "$line" ]; then
    misses=$((misses + 1))
    echo "[eval] snapshot unavailable (${misses} in a row)"
    [ "$misses" -ge 3 ] && { echo "[eval] host unreachable 3x -- check it"; exit 1; }
  ...
  echo "[eval] done=${done_}/${EXPECT} live=${sessions} csvs=${frames} rows=${ckpt} gpu0=${g0} gpu1=${g1} -- ${note}"

  if [ "${fail:-0}" -gt 0 ]; then
    echo "[eval] FAILURES in logs (tracebacks/OOM/nonzero exit): ${fail}"
  fi
  if [ "${done_:-0}" -ge "$EXPECT" ]; then
    echo "[eval] ALL ${EXPECT} JOBS FINISHED (frames=${frames} ckpt=${ckpt})"; exit 0
  fi
  if [ "${sessions:-0}" -eq 0 ]; then
    echo "[eval] NO SESSIONS LEFT but only ${done_}/${EXPECT} done -- jobs died"; exit 1
  fi
  if [ "$stalls" -ge 4 ]; then
    echo "[eval] STALLED across 4 intervals -- needs a look"; exit 1
  fi
  sleep "$INTERVAL"
done
