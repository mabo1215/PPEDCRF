...
def main() -> int:
    ...
    caption = (
        ...
        rf"worse privacy''; the interval is bootstrapped over places and the "
        rf"figure beside $p$ counts the discordant pairs the signed-rank test "
        rf"runs on. $p_{{\mathrm{{TOST}}}}$ is the equivalence test at the "
        rf"declared $\pm{args.margin:.2f}$ margin. Two things are readable "
        rf"here that the manuscript can only assert: at the operating point no "
        rf"operator lets placement separate, so the null is a property of the "
        rf"regime rather than of the additive-Gaussian operator the mechanisms "
        rf"we audit inherit; and at the large budget the placement effect "
        rf"reverses ``sign'' between operators --- edges help under "
        rf"additive noise and hurt under low-pass and block quantisation --- "
        rf"so a placement rule tuned on one operator can harm another. The "
        rf"uniform column also carries the threefold spread the manuscript "
        rf"quotes at that budget, from additive noise down to block "
        rf"quantisation.")
