...
def main() -> int:
    ...
    caption = (
        ...
        r"release. $k{=}1$ fine-tunes the last residual block, the setting the "
        r"main text reports; $k{=}5$ unfreezes every residual stage and the "
        r"stem. Both ran in one batch under one protocol rather than compared "
        r"across runs: each adapted model is scored on the held-out split its "
        r"own checkpoint trained against, differenced against an unadapted "
        f"baseline on that same split, over {len(seeds)} seeds. Cells pool the "
        r"seeds, since at one seed the split is 100 queries and cells swing "
        r"enough to flip sign; the parenthesised figure counts seeds moving "
        r"with the pooled mean, and the interval is bootstrapped over places. "
        r"Capacity is not what helps: at the pretrained index the whole "
        r"encoder buys nothing, and becomes worth about half of what the "
        r"perturbation removed only once the attacker may re-embed the "
        r"gallery.")
