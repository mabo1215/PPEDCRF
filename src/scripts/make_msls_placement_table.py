...
def main() -> int:
    ...
    caption = (
        ...
        rf"identical queries, so ``positive means worse privacy''. Both units "
        rf"of inference are printed; they barely differ here, $206$ places "
        rf"carrying a single query. ``Verdict'' is ``negligible'' when both "
        rf"intervals lie inside the $\pm{args.margin:.2f}$ margin declared in "
        rf"advance, ``none det.'' when they do not and the difference is not "
        rf"significant. The figure beside $p$ counts the discordant pairs the "
        rf"signed-rank test runs on, which bounds its power: a row with a "
        rf"handful reports an absent effect rather than a tested one. "
        rf"$p_{{\mathrm{{TOST}}}}$ is the equivalence $p$ at the same margin, "
        rf"a place-clustered bootstrap; it certifies four cells where the "
        rf"interval rule certifies two, and the verdict column keeps the more "
        rf"conservative pre-registered rule. Holm over these nine leaves every "
        rf"corrected $p$ at $1.000$, and a place-clustered permutation test "
        rf"agrees with the signed-rank column in every cell. The learned row "
        rf"is the mechanism's own map, at this checkpoint the uniform "
        rf"reference, so a self-comparison rather than a finding; the last two "
        rf"rows are the placement the margin analysis nominates and its "
        rf"inverse.")
    lines = [
