...
def main() -> int:
    ...
    lines = [
        ...
        rf"seeds over 200 images and privacy the two of {n_q} queries whose runs",
        r"share one optimiser configuration at every budget, the third having run",
        r"at 200 queries and a different random start. Drop is mIoU lost on those",
        r"images with a paired image-bootstrap interval; Top-1 carries a",
        r"query-bootstrap one. ``Hardened'' is the direction optimised in",
        r"expectation over attacker-side transforms. A drop interval lies within",
        r"a tolerance ($\checkmark$), straddles it ($\sim$) or lies beyond it",
        rf"(---); the declared tolerance is ${tol:.2f}$ and ${strict:.3f}$ the",
        r"stricter alternative of \S\ref{sec:direction}, both calibrated against",
        r"the spread between published segmenters on these VOC images; neither",
        r"is carried onto the agreement measurement on MSLS query frames in",
        r"\S\ref{sec:direction}, which is a different metric on a different",
        r"class-size profile and settles the ordering rather than the",
        r"admissibility. The",
        r"unperturbed query scores Top-1 $0.210$ and the isotropic control",
        r"$0.197$. Retrieval is on MSLS and utility on VOC, paired by budget.}",
